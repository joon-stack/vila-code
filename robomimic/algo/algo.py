...
class RolloutPolicy(object):
    ...
    def _prepare_observation(self, ob, batched_ob=False, postprocess_visual_obs=True):
        ...
        # print("INFO: self.obs_normalization_stats: ", self.obs_normalization_stats)
        ...
        if postprocess_visual_obs:
            for k in ob:
                is_rgb = ObsUtils.key_is_obs_modality(key=k, obs_modality="rgb")
                # print(f"INFO: key={k}, is_rgb={is_rgb}, shape={ob[k].shape}, min={ob[k].min()}, max={ob[k].max()}")
                if ObsUtils.key_is_obs_modality(key=k, obs_modality="rgb") or ObsUtils.key_is_obs_modality(key=k, obs_modality="depth"):
                    ...
                    # print(f"DEBUG: After process_obs: min={ob[k].min()}, max={ob[k].max()}")
        return ob
