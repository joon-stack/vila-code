...
def optimizer_from_optim_params(net_optim_params, net):
    ...
    print("INFO: initial lr: ", lr)

    # Detect encoder modules that should be placed in a separate parameter group.
    class_encoders = []
    for module in net.modules():
        if module.__class__.__name__ == "LAOMEncoder" and hasattr(module, '_freeze') and not module._freeze:
            class_encoders.append(module)

    # Build the parameter groups.
    if class_encoders:
        class_encoder_params = set()
        for encoder in class_encoders:
            class_encoder_params.update(encoder.parameters())
        
        params = [
            {'params': list(class_encoder_params), 'lr': lr * 0.1},
            {'params': [p for p in net.parameters() if p not in class_encoder_params], 'lr': lr}
        ]
        print("INFO: Fine tuning with lr_scale 0.1 confirmed!!!!")
    else:
