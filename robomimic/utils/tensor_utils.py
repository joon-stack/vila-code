...
def time_distributed(inputs, op, activation=None, inputs_as_kwargs=False, inputs_as_args=False, **kwargs):
    ...
    # print("INFO: inputs shape, min, max, mean, std in time_distributed", inputs['obs']['agentview_image'].shape, inputs['obs']['agentview_image'].min().item(), inputs['obs']['agentview_image'].max().item(), inputs['obs']['agentview_image'].mean().item(), inputs['obs']['agentview_image'].std().item())
    if inputs_as_kwargs:
        ...
        outputs = op(inputs, **kwargs)
    # print("INFO: outputs keys in time_distributed", outputs.keys())
    # print("INFO: outputs shape, min, max, mean, std in time_distributed", outputs.shape, outputs.min().item(), outputs.max().item(), outputs.mean().item(), outputs.std().item())

    if activation is not None:
        outputs = map_tensor(outputs, activation)
    # print("INFO: outputs after activation shape, min, max, mean, std in time_distributed", outputs.shape, outputs.min().item(), outputs.max().item(), outputs.mean().item(), outputs.std().item())
    outputs = reshape_dimensions(outputs, begin_axis=0, end_axis=0, target_dims=(batch_size, seq_len))
    # print("INFO: outputs after reshape_dimensions shape, min, max, mean, std in time_distributed", outputs.shape, outputs.min().item(), outputs.max().item(), outputs.mean().item(), outputs.std().item())
    return outputs
