...
def get_exp_dir(config, auto_remove_exp_dir=False, resume=False):
    ...
    # Check if experiment name is already a timestamp (14 digits)
    if config.experiment.name.isdigit() and len(config.experiment.name) == 14:
        # experiment name is already a timestamp, use it directly
        base_output_dir = os.path.join(base_output_dir, config.experiment.name)
        time_str = ""  # no additional timestamp needed
    else:
        # experiment name is not a timestamp, create subdirectory structure
        base_output_dir = os.path.join(base_output_dir, config.experiment.name)
        # Skip timestamp subdirectory - create folders directly under experiment name
        time_str = ""
    
    if resume:
        assert os.path.exists(base_output_dir), "Resuming training run, but output dir {} does not exist".format(base_output_dir)
        # No need to find timestamp subdirectory since we're not using it
...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None, obs_normalization_stats=None):
    ...
    for _ in LogUtils.custom_tqdm(range(num_steps)):
        ...
        #     try:
        #         print("INFO: input_batch keys:", k, v.shape, v.min(), v.max())
        #     except:
        #         print("INFO: input_batch keys:", k, type(v))
        #         try: 
        #             print("v.keys():", v.keys())
        #             for kk, vv in v.items():
        #                 print("INFO: input_batch keys:", kk, vv.shape, vv.min(), vv.max())
        #         except:
        #             pass
