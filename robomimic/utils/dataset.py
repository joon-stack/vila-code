...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # load demo information (optionally restricted by provided demos)
        self.load_demo_info(filter_by_attribute=self.filter_by_attribute, demos=demos, demo_limit=demo_limit)
    ...
    def load_demo_info(self, filter_by_attribute=None, demos=None, demo_limit=None):
        ...
        if demo_limit is not None:
            ...
            print("INFO: demo_limit is set to {}, so the number of demos is limited to {}".format(demo_limit, len(self.demos)))
...
class MultiViewDataset(SequenceDataset):
    """
    A Dataset class for multi-view datasets. This class should be used in place
    of SequenceDataset for datasets that contain multiple camera views.

    It effectively multiplies the dataset size by the number of views, treating
    each view as a separate sample. For each sample, it selects one camera view
    and places it under a canonical key.
    """
    def __init__(self, *args, canonical_key="agentview_image", resize_wh=None, max_samples=None, **kwargs):

        # IMPORTANT: disable 'all' caching in parent, because we need to modify the item
        # before it would be cached.
        if kwargs.get("hdf5_cache_mode") == "all":
            print("MultiViewDataset WARNING: hdf5_cache_mode='all' is not supported. Disabling getitem caching.")
            kwargs["hdf5_cache_mode"] = "low_dim" # fallback to low_dim, which is safe

        # Handle max_samples parameter
        if max_samples is not None:
            # Convert max_samples to demo_limit for the parent class
            # Since MultiViewDataset multiplies dataset size by num_views,
            # we need to adjust demo_limit accordingly
            kwargs["demo_limit"] = max_samples

        super().__init__(*args, **kwargs)
        self.canonical_key = canonical_key
        self.resize_wh = resize_wh
        self.max_samples = max_samples
        # Identify RGB view keys from the obs_keys it was initialized with
        self.view_keys = sorted([
            k for k in self.obs_keys
            if k.startswith("view_") and ObsUtils.key_is_obs_modality(k, "rgb")
        ])

        if not self.view_keys:
            raise ValueError("No view keys (e.g., 'view_00/rgb') found in obs_keys for MultiViewDataset.")

        self.num_views = len(self.view_keys)
        self.original_dataset_len = super().__len__()

        # Identify non-view observation keys that will be kept
        self.other_obs_keys = [k for k in self.obs_keys if k not in self.view_keys]
        
        # The parent may have created a getitem_cache. We must invalidate it.
        if hasattr(self, "getitem_cache") and self.getitem_cache is not None:
            self.getitem_cache = None
            print("MultiViewDataset NOTE: disabled parent getitem_cache.")
                
    def _resize_single_image(self, img_array):
        """
        Resize a single 3D image array (H, W, C) using PIL.
        """
        # Ensure we have a 3D array (H, W, C)
        ...
        img_resized = img_pil.resize(self.resize_wh, Image.LANCZOS)
        
        # Convert back to numpy array
        img_resized_array = np.array(img_resized)
        
        # Convert back to (C, H, W) format if original was (C, H, W)
        if len(img_array.shape) == 3 and img_array.shape[0] == 3:
            img_resized_array = np.transpose(img_resized_array, (2, 0, 1))
        
        return img_resized_array

    def _resize_image(self, img_array):
        """
        Resize image array using PIL if resize_wh is specified.
        
        Args:
            img_array (np.ndarray): Image array with shape (H, W, C) or (C, H, W)
            
        Returns:
            np.ndarray: Resized image array
        """
        if self.resize_wh is None:
            return img_array
        ...
        # Handle 4D data (T, H, W, C) - resize each frame individually
        if img_array.ndim == 4:
            T, H, W, C = img_array.shape
            resized_frames = []
            
            # Resize each frame individually
            for t in range(T):
                frame = img_array[t]  # (H, W, C)
                resized_frame = self._resize_single_image(frame)
                resized_frames.append(resized_frame)
            
            # Stack all resized frames back to 4D
            resized_4d = np.stack(resized_frames, axis=0)  # (T, new_H, new_W, C)
            return resized_4d
        
        # Handle 3D data
        img_array = np.squeeze(img_array)
        
        # Use the helper function for 3D data
        try:
            resized_array = self._resize_single_image(img_array)
            return resized_array
        except Exception as e:
            print(f"ERROR in _resize_image: {e}")
            print(f"Image shape: {img_array.shape}, dtype: {img_array.dtype}")
            return img_array

    def __len__(self):
        return self.original_dataset_len * self.num_views

    def get_item(self, idx):
        # Override get_item to select a view
        original_idx = idx // self.num_views
        view_idx = idx % self.num_views
        selected_view_key = self.view_keys[view_idx]
        
        # Build obs_keys using the selected view plus the other non-view observations.
        # This loads only one view and significantly reduces I/O and memory usage.
        selected_obs_keys = self.other_obs_keys + [selected_view_key]
        
        # Reuse the parent get_item logic while loading only the selected view.
        demo_id = self._index_to_demo_id[original_idx]
        demo_start_index = self._demo_id_to_start_indices[demo_id]
        demo_length = self._demo_id_to_demo_length[demo_id]
        
        # start at offset index if not padding for frame stacking
        demo_index_offset = 0 if self.pad_frame_stack else (self.n_frame_stack - 1)
        index_in_demo = original_idx - demo_start_index + demo_index_offset
        
        # end at offset index if not padding for seq length
        demo_length_offset = 0 if self.pad_seq_length else (self.seq_length - 1)
        end_index_in_demo = demo_length - demo_length_offset
        
        # Load dataset keys such as actions and rewards.
        meta = self.get_dataset_sequence_from_demo(
            demo_id,
            index_in_demo=index_in_demo,
            keys=self.dataset_keys,
        ...
        # Load only the selected view.
        meta["obs"] = self.get_obs_sequence_from_demo(
            demo_id,
            index_in_demo=index_in_demo,
            keys=tuple(selected_obs_keys),  # Load only the selected keys instead of all views.
            num_frames_to_stack=self.n_frame_stack - 1,
            seq_length=self.seq_length,
            prefix="obs"
        )
        ...
        # Process 'obs' by remapping the selected view to the canonical key.
        if "obs" in meta:
            new_obs = {}
            # Copy over non-view observations
            for k in self.other_obs_keys:
                if k in meta["obs"]:
                    new_obs[k] = meta["obs"][k]
            # Add the selected view image under the canonical key
            if selected_view_key in meta["obs"]:
                # Skip resize here - let the encoder handle it
                img = meta["obs"][selected_view_key]
                new_obs[self.canonical_key] = img
            meta["obs"] = new_obs
        
        # Process 'next_obs'
        if self.load_next_obs and "next_obs" in meta:
            new_next_obs = {}
            for k in self.other_obs_keys:
                if k in meta["next_obs"]:
                    new_next_obs[k] = meta["next_obs"][k]
            if selected_view_key in meta["next_obs"]:
                # Skip resize here - let the encoder handle it
                img = meta["next_obs"][selected_view_key]
                new_next_obs[self.canonical_key] = img
            meta["next_obs"] = new_next_obs
            
        return meta
    
    def __getitem__(self, idx):
        # The parent __getitem__ just calls get_item or uses cache.
        # Since we disabled the cache that matters, we can just call our own get_item.
