...
VALID_IMAGE_CHANNEL_DIMS = {1, 3, 4}       # depth, rgb, rgbd
...
def process_frame(frame, channel_dim, scale):
    ...
    Args:
        frame (np.array or torch.Tensor): frame array or pre-extracted feature
        channel_dim (int): Number of channels to sanity check for
        scale (float or None): Value to normalize inputs by

    Returns:
        processed_frame (np.array or torch.Tensor): processed frame or feature
    """
    # Check if this is a pre-extracted feature
    # if frame.ndim <= 2:
    #     # Pre-extracted feature: pass through without processing
    #     return frame
    
    # Channel size should either be 3 (RGB) or 1 (depth)
    assert (frame.shape[-1] == channel_dim)  # Kept for the standard image path; fall back below for feature tensors.
    if frame.shape[-1] != channel_dim:
        # If shape doesn't match expected, assume it's pre-extracted feature
        return frame
...
class ImageModality(Modality):
    ...
    def _default_obs_processor(cls, obs):
        ...
        For pre-extracted features, passes through without processing.

        Args:
            obs (np.array or torch.Tensor): image array or pre-extracted feature

        Returns:
            processed_obs (np.array or torch.Tensor): processed image or feature
        """
        # Support RGBD packed into a single 4-channel image: RGB normalized to [-1, 1], depth left raw.
        if obs.shape[-1] == 4:
            obs = TU.to_float(obs)
            if isinstance(obs, np.ndarray):
                ...
                obs = np.concatenate([rgb, depth], axis=-1)
            else:
                rgb = obs[..., :3]
                depth = obs[..., 3:]
                rgb = (rgb / 255.0 - 0.5) * 2.0
        ...
        # # Check if this is a pre-extracted feature
        # if obs.ndim == 1 or (obs.ndim == 2 and obs.shape[-1] <= 512):
        #     # Pre-extracted feature: pass through without processing
        #     return obs
        # elif obs.ndim >= 3:
        #     # Actual image: use original processing
        #     return process_frame(frame=obs, channel_dim=3, scale=255.)
        # else:
    ...
    def _default_obs_unprocessor(cls, obs):
        ...
        #     return obs
        # else:
            # Actual image: use original unprocessing
            # return TU.to_uint8(unprocess_frame(frame=obs, channel_dim=3, scale=255.))
        # Handle RGBD packed into 4 channels: RGB in [-1, 1], depth raw.
        if obs.shape[-3] == 4:
            obs = batch_image_chw_to_hwc(obs)
            if isinstance(obs, np.ndarray):
                rgb = obs[..., :3]
                depth = obs[..., 3:]
                rgb = ((rgb / 2.0) + 0.5) * 255.0
                return np.concatenate([rgb, depth], axis=-1).astype(np.float32)
            else:
                rgb = obs[..., :3]
                depth = obs[..., 3:]
                rgb = ((rgb / 2.0) + 0.5) * 255.0
                return torch.cat([rgb, depth], dim=-1).float()
