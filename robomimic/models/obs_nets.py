...
# for LAOMEncoder
try:
    from vila_stage1.nn import Actor as LAOM_Actor
    from vila_stage1.utils import normalize_img as laom_normalize_img
    LAOM_AVAILABLE = True
except ImportError:
    LAOM_Actor = None
    laom_normalize_img = None
    LAOM_AVAILABLE = False
...
class ObservationEncoder(Module):
    ...
    def forward(self, obs_dict):
        ...
        for ind, k in enumerate(self.obs_shapes):
            # print("INFO: k: ", k)
            # print("INFO: self.obs_nets.keys(): ", self.obs_nets.keys())
            ...
            if self.obs_nets[k] is not None:
                ...
                if self.activation is not None:
                    if isinstance(self.activation, nn.ReLU):
                        ...
                        x = self.activation(x)
                    # print(f"  - After activation '{k}': min={x.min():.4f}, max={x.max():.4f}, mean={x.mean():.4f}")
            ...
            # print("INFO: x.shape, min, max, mean, std", x.shape, x.min().item(), x.max().item(), x.mean().item(), x.std().item())
            feats.append(x)
    ...
    def set_eval(self):
        ...
        """
        self.eval()
        for net in self.obs_nets.values():
            if net is not None:
                net.eval()
...
class ObservationGroupEncoder(Module):
    ...
    def forward(self, **inputs):
        ...
        for obs_group in self.observation_group_shapes:
            ...
            # print("INFO: obs_group: ", obs_group)
            # print("INFO: self.nets.keys(): ", self.nets.keys())
            # print("INFO: inputs[obs_group].keys(): ", inputs.keys())
