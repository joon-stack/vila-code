...
class _LAOMResidualBlock(nn.Module):
    """
    Residual block implementation from LAOM.
    Helper class for LAOMEncoder.
    """
...
class _LAOMEncoderBlock(nn.Module):
    """
    Encoder block implementation from LAOM.
    Helper class for LAOMEncoder.
    """
    def __init__(self, input_shape, out_channels, num_res_blocks=2, dropout=0.0, downscale=True):
        ...
        self.blocks = nn.Sequential(*[_LAOMResidualBlock(self._out_channels, dropout) for _ in range(num_res_blocks)])
...
class LAOMEncoder(ConvBase):
    """
    LAOM (Latent Action-Observation Model) encoder for robomimic.
    This class wraps the full LAOM Actor model to output a latent action vector directly,
    similar to how R3MConv and MVPConv work.
    """
    def __init__(
        self,
        input_channel=3,
        model_path=None,
        freeze=True,
        # These are now defaults and will be overridden by the config file
        ...
        **kwargs
        ...
        super(LAOMEncoder, self).__init__()
        
        # Note: No preprocessing needed as data is already 64x64 in the dataset
        # self.preprocess = transforms.Resize((64, 64), antialias=True)  # Removed for performance
        ...
        # Default architecture values, to be overridden by config file
        self.latent_action_dim = latent_action_dim
        self._encoder_scale = encoder_scale
        self._encoder_channels = encoder_channels
        self._encoder_num_res_blocks = encoder_num_res_blocks
        self._dropout = dropout
        self._pretrained = model_path is not None
        
        if self._pretrained:
            self._load_arch_from_config(model_path)

        # Build the full LAOM Actor architecture
        shape = (self._input_channel, 64, 64)
        conv_stack = []
        for out_ch in self._encoder_channels:
            conv_seq = _LAOMEncoderBlock(shape, self._encoder_scale * out_ch, self._encoder_num_res_blocks, self._dropout)
            shape = conv_seq.get_output_shape()
            conv_stack.append(conv_seq)
        
        self.encoder = nn.Sequential(*conv_stack)
        
        # This is the head that produces the latent action vector
        self.actor_mean = nn.Sequential(
            nn.ReLU6(),
            nn.Linear(shape[0], self.latent_action_dim),
        )
    ...
    def forward(self, inputs):
        """
        Forward pass: normalize, then pass through the full LAOM Actor
        to get the latent action vector.
        Note: Data is already resized to 64x64 in the dataset, so we skip resize here.
        """
        if not self.dummy:
        # 1. Normalize the image (data is already 64x64)
            x = (inputs - 0.5) * 2.0
            x = self.encoder(x)

            # 3. Global average pooling equivalent and final linear layer from LAOM Actor
    ...
    def _load_arch_from_config(self, model_path):
        """Load architecture parameters from the config file associated with the checkpoint."""
        config_path = os.path.join(os.path.dirname(model_path), 'config.yaml')
        if not os.path.exists(config_path):
            # If no config is found, we cannot proceed as latent_action_dim is unknown.
            raise FileNotFoundError(f"CRITICAL: Config file not found at {config_path}. Cannot determine LAOM architecture.")

        try:
            with open(config_path, 'r') as f:
                config_data = yaml.safe_load(f)

            # Directly access the necessary keys from the nested structure.
            if 'lapo' in config_data and 'latent_action_dim' in config_data['lapo']:
                self.latent_action_dim = config_data['lapo']['latent_action_dim']
            else:
                raise KeyError("'lapo' section with 'latent_action_dim' not found in config file")

            if 'bc' in config_data:
                bc_config = config_data['bc']
                self._encoder_scale = bc_config.get('encoder_scale', self._encoder_scale)
                self._encoder_num_res_blocks = bc_config.get('encoder_num_res_blocks', self._encoder_num_res_blocks)
                self._dropout = bc_config.get('dropout', self._dropout)
                encoder_deep = bc_config.get('encoder_deep', False)
                self._encoder_channels = (16, 32, 64, 128, 256) if encoder_deep else (16, 32, 32)
            else:
                print("Warning: 'bc' section not found in config. Using default encoder architecture params.")

            print(f"Successfully loaded LAOMEncoder architecture from {config_path}")
            print(f"  - latent_action_dim: {self.latent_action_dim}")
            print(f"  - encoder_scale: {self._encoder_scale}")
            print(f"  - encoder_channels: {self._encoder_channels}")

        except KeyError as e:
            print(f"Error: Missing critical key {e} in config file {config_path}.")
            raise
        except Exception as e:
            print(f"Error: Failed to load or parse arch from {config_path}: {e}.")
            raise

    def _load_checkpoint(self, model_path):
        """Load LAOM Actor checkpoint for the whole internal model."""
        if not self.dummy:
            try:
                ...
                # Since we are loading the full actor, the keys should match directly.
                self.load_state_dict(state_dict, strict=True)
                print(f"LAOM full Actor weights loaded successfully from {model_path}")
                    
            except Exception as e:
                print(f"Error: Failed to load LAOM checkpoint from {model_path}.")
                raise e
        else:
            pass
    ...
    def output_shape(self, input_shape):
        """
        Return the shape of the latent action vector.
        """
    ...
    def __repr__(self):
        """Pretty print network."""
        header = '{}'.format(str(self.__class__.__name__))
        return header + '(input_channel={}, latent_action_dim={}, freeze={}, pretrained={})'.format(
            self._input_channel, self.latent_action_dim, self._freeze, self._pretrained
        )
